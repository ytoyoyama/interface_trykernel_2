...
ER tk_def_int( UINT intno, const T_DINT *pk_dint )
{
    ...
    if((intno >= N_INTVEC && intno < 100)|| intno >= N_INTVEC+100) return E_ID;

    if(intno >= 100) {      // 100以上はCPUコア1の割り込み
        coreno = 1;
        intno -= 100;
    }
    ...
    if(inthdr != NULL) {
        if ( (pk_dint->intatr & TA_HLNG) != 0 ) {   // TA_HLNG属性のハンドラ処理
            inthdr_tbl[coreno][intno] = inthdr;     // TA_HLNG属性の割り込みハンドラの登録
            inthdr = hll_inthdr;
        }		
    } else 	{
    ...
    // 割込みハンドラの登録
    intvet = coreno?(FP*)&vec_tbl_c1[N_SYSVEC]:(FP*)&vec_tbl_c0[N_SYSVEC];
    intvet[intno] = inthdr;
...
void EnableInt( UINT intno, INT level )
{
    ...
    if((intno < 100 && CPU_CORE != 0) || (intno >= 100 && CPU_CORE != 1)) {
        icc_ras_int( ICCINT_ENAINT<<24 | level<<16 | intno);
        return;
    }
    if(intno >= 100) intno -= 100;
...
{
    if((intno < 100 && CPU_CORE != 0) || (intno >= 100 && CPU_CORE != 1)) {
        icc_ras_int( ICCINT_DISINT<<24 | intno);
        return;
    }
    if(intno >= 100) intno -= 100;
...
/* CPUコア間のEnableInt (CPUコア間割り込みハンドラ) */
void iccint_ebableint(UW data)
{
    EnableInt( data & 0x0000FFFF, (data>>16)&0x0F);
}

/* CPUコア間のDisableInt (CPUコア間割り込みハンドラ)*/
void iccint_disableint(UW data)
{
    DisableInt(data & 0x0000FFFF);
}