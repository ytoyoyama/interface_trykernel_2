...
typedef enum {
    ...
    TWFCT_ICMR  = 101,  // icc_rcv_msgによる受信待ち
} TWFCT;
...
/* CPUコア間割込み制御関数 */
void init_icc_int(void);                  // CPUコア間割込み初期化
...
typedef struct st_icmcb {
    ...
    SZ      freesz;         // バッファ・サイズ
    void    *buf_rp;        // バッファ・ライトポイント
    void    *buf_wp;        // バッファ・リードポイント
} ICMCB;
